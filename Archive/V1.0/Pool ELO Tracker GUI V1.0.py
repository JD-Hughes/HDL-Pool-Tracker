...
class EloApp:
    ...
    def record_match(self):
        ...
        # Calculate dynamic K based on games played
        ...
        loser_elo_before = loser_elo_current
        loser_elo_after = loser_elo
        winner_elo_diff = winner_elo - winner_elo_current
        loser_elo_diff = loser_elo_after - loser_elo_before
        
        summary = (
            f"{now_str} | {winner} def. {loser} | "
            f"+{winner_elo_diff} → {winner_elo} / {loser_elo_diff} → {loser_elo_after}\n"
            f"(K-factor used: {k})"
        )
